 ...
 * The Mysterion method ... partitions an array into 2 arrays (one whose vals <
 * partition, the other whose vals > partition)
 * <p>
...
public class Mysterion
{
   ...
   *
   */
  public static int mysterion( int arr[], int loBound, int hiBound, int splitIndex)
  {
    int splitVal = arr[splitIndex];
    swap(splitIndex, hiBound, arr); // moves splitVal to the end
    int s = loBound;
    for (int i = loBound; i < hiBound; i++){
      if (arr[i] < splitVal){ // moves values < splitVal to next spot where a swap hasn't already happened
        swap(s, i, arr);
        s += 1;
      }
    }
